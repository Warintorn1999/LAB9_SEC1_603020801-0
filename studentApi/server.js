...
app.get('/allstd',function(req,res){
    dbConn.query('SELECT * FROM Student',function(error,result,fields){
...
app.put('/std/:id',function (req,res){
    ...
    dbConn.query("UPDATE student SET ? WHERE std_id = ?",[std,std_id],function (error,result,fields){
        if (error) throw error;
        return res.send({ error: false,data: result,message:'Student has been updated successfully.'});
    });
});
//Delete student
app.delete('/std/:id',function (req,res){
    ...
    dbConn.query("DELETE FROM student WHERE std_id = ?",std_id,function (error,result,fields){
        if (error) throw error;
        return res.send({ error: false,data: result,message:'Student has been deleted successfully.'});
    });
});
//set port
